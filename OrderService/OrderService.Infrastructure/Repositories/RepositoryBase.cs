...
{
    public class RepositoryBase<T>(OrderDbContext context) : IReadRepository<T>, IWriteRepository<T> where T : BaseDomainModel
    {
        ...
        private static string GetIncludePath(Expression<Func<T, object>> includeExpression)
        {
        ...
        private static void VisitExpression(Expression expression, IList<string> parts)
        {
            switch (expression)
            {
